...
BM_input_dict = {
    "dispatch_ih": [[1, 2, 3, 4, 5], [10, 20, 30, 40, 50], [20, 30, 40, 50, 60]],# MWh (per actor per hour for a whole year), int, 2D array
    "price_h" : [1, 2, 3, 4, 5], # EUR/MWh (per actor per hour for a whole year) float, 1D array
    "opcost_ih" : [[0, 0, 0, 0, 0], [1, 2, 3, 4, 5], [2, 30, 40, 5, 60]],# EUR (per hour per actor for a whole year), float, 2D array 
    ...
    "rls" : [[0, 0], [0, 2], [1, 1], [1, 3], [1, 6], [2, 4], [2, 5]]  ### important connects acors (first col) with different techs (second col)
}
